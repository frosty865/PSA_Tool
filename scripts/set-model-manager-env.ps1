# Set environment variables for VOFC-ModelManager service from .env file
# Run this after installing the service to configure environment variables
# Must be run as Administrator

param(
    [string]$ServiceName = "VOFC-ModelManager"
)
...
$POSSIBLE_ROOTS = @(
    ...
    "C:\Tools\PSA_Tool",
    "C:\Tools\py_scripts\model_manager"
)
...
Get-Content $ENV_FILE | ForEach-Object {
    ...
    if ($_ -match '^\s*([^#][^=]+)=(.*)$') {
        ...
        # Only include relevant variables for model manager
        # Map SUPABASE_SERVICE_ROLE_KEY to SUPABASE_KEY for compatibility
        ...
        elseif ($key -match '^(SUPABASE_|NEXT_PUBLIC_SUPABASE_|OLLAMA_|VOFC_)' -and 
...
if ($envVars.Count -eq 0) {
    ...
    Write-Host "Looking for: SUPABASE_URL, SUPABASE_KEY, SUPABASE_SERVICE_ROLE_KEY, OLLAMA_BASE_URL" -ForegroundColor Yellow
    exit 1
}
